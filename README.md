...
**Implementation:** `updateReputation(peerId, delta, reason)`, `getReputation(peerId)`, `getThrottleState(peerId)`, `isByzantineIsolated(peerId)`. Message handler checks throttle state before processing (the `handleMessage` closure inside `_attach()`, the `getThrottleState(senderId)` sender-keyed accept/queue/drop branch, plus a cheaper `getThrottleState(connKey)` connection-keyed drop-only pre-check before signature/PoW work is spent). Convergence time: < 1 second on 10-peer network. Witness: `tools/witness-reputation-ledger.js` validates all delta rules and throttle states.

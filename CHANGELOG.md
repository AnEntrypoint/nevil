...
#### Eleven Real Bugs Fixed (fourth audit pass — exhaustive shortcomings sweep, wfgy-method discovery)

Found via a deliberate `/wfgy-method`-driven audit request; every fix witnessed live via direct node execution plus the full existing `tools/witness-*.js` suite (21 scripts, all pass, all exit cleanly). Two of the eleven were only discovered mid-fix when the earlier fixes broke `tools/witness-integration.js`, which is exactly the adversarial-verification this audit pass exists to catch.

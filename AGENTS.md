...
- `close()` — async; tears down the network layer (all open sockets, the queue-drain timer, and any attached `WebSocketServer`), unsubscribes graph listeners, and awaits `storage.close()`, which itself drains every log store's in-flight/queued write (main log, reputation log, peer-table log, and the soul index's pending disk-backed SSTable/manifest writes when `sstableDiskEnabled` is on) before releasing file/IndexedDB resources — a caller doing a graceful shutdown should `await close()` before exiting the process, or a write that already updated in-memory graph state (`graph.onAny`'s handler calls `storage.persist()` unawaited, since that handler is synchronous) can be lost even though it looked durable from every in-process signal; a caller that drops the returned promise behaves like the prior fire-and-forget synchronous `close()`, minus that guarantee. Safe to call before `ready()` resolves (the constructor kicks off boot asynchronously without awaiting it, so `this.network` may not exist yet)
...
- Config: `dhtK`, `dhtL`, `dhtGeohashLength`, `dhtFallbackThreshold`, `dhtEnabled`, `dhtHealthAverageWindow` (default 100 — bounds the moving-average sample count used by `updatePeerHealth()`, so a long-lived connection's health score stays responsive to recent latency/loss instead of an ever-growing denominator). Peer health is updated event-driven on real socket activity (open, message receive), not on a fixed interval — the previously-documented `dhtHealthUpdateFreq` knob was removed as dead (it was stored but never read; no periodic health updater ever consumed it)
- Reputation rewards are now actually awarded, not just penalties: a valid first-seen fully-gated `put` awards `REP_DELTA_GOOD` ('good') to its authenticated sender, and the first authenticated loop-back ACK of a message from a peer that was NOT a direct send target awards `REP_DELTA_ROUTING_HELP` ('routing-help') for proven real relay. Previously every internal `updateReputation` call was a penalty (malformed/byzantine/replay), so the ledger was a one-way ratchet toward `drop` and a well-behaved peer could never build a positive buffer — both reward constants were declared but never triggered anywhere. Positive reputation is bounded by `REP_MAX_POSITIVE` (`repMaxPositive`, default 20) so a peer cannot pre-farm an unbounded accept buffer from valid puts and then absorb an equally-unbounded run of Byzantine/malformed penalties without ever crossing the drop threshold; and the routing-help ACK award only fires on a loop-back whose digest matches a message THIS node actually broadcast (a `_sentDigests` hit) — a foreign-origin duplicate with no such entry can no longer forge the ACK to farm routing-help reputation or suppress the DHT-fallback reflood

### iroh QUIC Transport (optional, Node-only)
- `new Nevil({ irohEnabled: true, irohRelay, irohSecretKey, irohPeers })` — opt-in (default off). Adds an iroh (`@number0/iroh`) QUIC transport ALONGSIDE the ws transport: iroh sockets are `_attach()`ed exactly like ws sockets, so DHT routing, flood-fill, reputation throttle, PoW gate, signature verification, and `maxPayloadBytes` all apply identically — the transport swap changes bytes-on-wire only, never the admission pipeline (witnessed: a signed `putAt` propagates over iroh and `getAtVerified` confirms it on the peer with signatures intact; an unsolved put over iroh is dropped + penalized under `powEnabled` exactly as over ws)
- Why iroh: its Ed25519 `EndpointId` lines up directly with nevil's Ed25519 souls, and its relay-assisted hole-punching reaches peers behind NAT that a raw `ws://` dial cannot. `irohSecretKey` (a 32-byte Ed25519 seed) deterministically binds a node's transport identity — the same seed always yields the same `EndpointId`
- `nevil.irohNodeAddr()` — this node's dialable `EndpointAddr` (the iroh analogue of a `ws://` URL, but Ed25519-addressed and NAT-traversable), or `null` when `irohEnabled` is off. `nevil.dialIroh(endpointAddr)` — dial a peer at runtime; `irohPeers` dials at construction
- Framing: a QUIC BiStream is a raw byte stream, so each JSON envelope is length-prefixed (4-byte big-endian length + body); an oversized frame is refused before its body is read, symmetric with ws `maxPayload`. Writes are serialized so QUIC backpressure never interleaves frames. A peer connection is keyed by its Ed25519 `EndpointId` (stable across reconnects, unlike ws's ephemeral address:port), and a mutual-dial race is deduped to one socket per physical peer
- `irohRelay` (default true — n0 relay-assisted connectivity via `applyN0()`; set false for direct/LAN-only via `applyN0DisableRelay()`, which also installs the required rustls crypto provider). `@number0/iroh` is a native napi binding declared as an `optionalDependency` (attempt-but-not-fail install), so a ws-only/browser consumer's install never breaks on a platform lacking the binding and the hard runtime dependency stays `{sodium-universal}` only. `irohEnabled: false` (the default) never `require`s iroh, keeping the browser + two-dependency baseline fully intact; `irohEnabled: true` fails loud and specific (naming the dep + `npm install`) if the binding is absent
- Witness: `tools/witness-iroh-transport.js` — real 2-node iroh QUIC connection, node A does a signed `putAt`, node B's `getAtVerified` confirms it with signatures intact; skips cleanly (exit 0) when `@number0/iroh` is not installed so the witness suite stays green ws-only
...
- `localClock` only advances when `mergeNode` actually accepts at least one field (real data lands, `changed.length > 0`) — a message contributing zero accepted data (every field non-finite-ts/implausibly-future-ts, or every field losing HAM/FWW to already-newer local state) leaves `localClock` untouched, so a peer can't silently ratchet a victim's clock forward with a stream of garbage or losing writes
- `graph.TS_MAX_JUMP_MS` (`tsMaxJumpMs`, default 5 minutes) — bounds how far ahead of a field's own currently-stored `ts` an incoming write's `ts` may sit; a field beyond that ceiling is REJECTED (skipped, same as a non-finite `ts`), never clamped-and-stored. The ceiling is derived from the field's own recorded history (`currentTs + TS_MAX_JUMP_MS`), not the receiving peer's `Date.now()` — a receive-time-relative ceiling would let two honest peers merging the IDENTICAL message at different real moments land on different accept/reject outcomes for a borderline `ts` (one peer's wall clock has advanced past the cutoff by the time it merges, the other's hasn't), diverging on whether the field changes at all. Bounding against `currentTs` is deterministic because, like `localClock`, it only advances via messages actually merged, never via real elapsed time. A field with no prior `ts` yet (first write) has no history to protect and is checked only against a fixed absolute sanity constant (year 9999) instead, screening out `Number.MAX_SAFE_INTEGER`-class poisoning that `Number.isFinite` alone would miss
- Config: `clockMaxJump`, `tsMaxJumpMs`
...
- Config: `repAcceptThreshold`, `repDropThreshold`, `repDelta*`, `repMaxPositive` (default 20), `queueRetryDelay`, `queueMaxRetries` — `getThrottleState` is accept (`>= repAcceptThreshold`) / drop (`< repDropThreshold`) / queue (the band between); `repQueueMin` does not exist as a separate knob (removed as dead — it previously silently did the drop-cutoff job `repDropThreshold`'s name and docs already claimed)
...
- Incoming `reputationLedger` gossip entries are unauthenticated hearsay (any message reaching that point in `handleMessage` can carry one, including a fully unsigned put to a plain non-keychain soul — there is no proof binding a claimed `entry.peerId` to the reporting connection's real observations), so each entry is clamped to a narrower band than a locally-observed delta and both the number of distinct `peerId` targets AND the number of applied entries per target one connection may gossip about are rate-limited per window: `repGossipDeltaMin`/`repGossipDeltaMax` (default -5/10, matching the largest single local-observation delta magnitudes rather than the old flat +/-100), `repGossipMaxTargets` (default 20 distinct peerIds per connection per window), `repGossipMaxEntriesPerTarget` (default 5 — bounds repeated entries for the SAME peerId within one window, since the distinct-target limit alone only gates the first entry seen per peerId and does not stop a single oversized message from applying many clamped deltas to one victim via distinct fake `timestamp`s that each defeat the entryKey dedup), `repGossipWindowMs` (default 60000)
...
- `network.reputationCache` (the `peerId -> cumulative score` Map) is FIFO-bounded by `maxReputationLedger` the same as `seen`/`reputationLedger`, via `_setReputationCache()` at every write site — a peer gossiping a stream of fabricated `peerId`s can't grow the cache without limit. Eviction skips any peerId currently below `repAcceptThreshold` when choosing a victim (evicting the oldest only if every candidate is pinned), so FIFO pressure from a flood of fabricated peerIds can never silently reset a Byzantine/drop-throttled peer's score back to neutral (`accept`) — the documented "Byzantine peer's throttle state survives" guarantee holds under remotely-inducible cache churn, not just across restart
...
- Network: DHT_K, DHT_L, DHT_GEOHASH_LENGTH, DHT_FALLBACK_THRESHOLD, DHT_ENABLED, `dhtHealthAverageWindow` (default 100)
- Network transport: `maxPayloadBytes` (default 1MB — caps the raw frame size at both `WebSocketServer`/client-dial sites AND the iroh length-prefix framing, independent of/orthogonal to `graph.js`'s field-count cap `maxFieldsPerNode`), `redialBaseMs`/`redialMaxMs` (default 2000ms/60000ms — base/cap for `_redialDelay()`'s exponential-backoff reconnect scheduling of a configured peer that disconnects), `backpressureUpdateIntervalMs` (default 50ms — throttles how often `_relay()` recomputes the adaptive `writeRateScale` backpressure signal, since recomputing it by re-sorting the latency-sample buffers on every single relayed message added O(n log n) work per message for a signal that only meaningfully moves on the timescale of real latency change)
- iroh transport (optional, Node-only): `irohEnabled` (default false), `irohRelay` (default true), `irohSecretKey` (32-byte Ed25519 seed, defaults to an ephemeral key), `irohPeers` (array of dialable `EndpointAddr` objects). See the "iroh QUIC Transport" section above
- Graph: CLOCK_MAX_JUMP, `tsMaxJumpMs` (wall-clock ts ceiling, default 5 minutes)
- Reputation: `repAcceptThreshold`, `repDropThreshold`, `repDelta*`, `repMaxPositive` (default 20), `queueRetryDelay`, `queueMaxRetries`
...
- The iroh QUIC transport (`irohEnabled`) is opt-in and Node-only: `@number0/iroh` is a native napi binding, so it neither runs in-browser nor is required by the default ws path — it is an `optionalDependency` (not a hard `dependency`), and the two-runtime-dependency baseline (`sodium-universal` hard, `ws` optional peer) is unchanged. iroh adds real QUIC transport with relay-assisted NAT hole-punching and Ed25519-`EndpointId` addressing that aligns with nevil's souls; it does NOT replace the application-layer gossip/DHT/CRDT — those ride on top of iroh connections exactly as they do over ws (iroh v1.0.0's core binding exposes `Endpoint`/`Connection`/`BiStream` but not `iroh-gossip`, so nevil's own flood-fill/bounded-subset routing remains the gossip layer). It is a small-to-medium mesh transport upgrade, not a Kademlia-proof structured overlay.
